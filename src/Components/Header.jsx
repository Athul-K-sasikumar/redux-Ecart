...
const Header = ({insideHome}) => {
  ...
  return (
     <nav className='flex w-full bg-blue-950 fix top-0 p-5 items-center' >
      <Link className=' text-white font-bold ' to={'/'}><i class="fa-solid fa-truck-fast me-1"></i>E Cart</Link>
      <ul className='flex-1 text-right '>
       {insideHome && 
         <li className='list-none inline-block px-5 '><input onChange={e=>dispatch(searchProduct(e.target.value.toLocaleLowerCase() ))} style={{width:'300px'}} type="text" className='rounded-full p-1 text-center' placeholder='Sreach the products here!' /></li>
       }
